...
export async function runTopLevelAgentLoop(
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      // Summarization has its own budget: inheriting a 256-token ordinary-turn
      // cap produces truncated summaries and repeated full-context requests.
      // Never retry compaction by shrinking its budget below a useful summary.
      if (compacting) {
      ...
      const failWithoutRetry =
        compacting || (!isContextLengthError(attemptResult.error) && isNonRetryableLLMError(attemptResult.error))
    ...
    // A partial, reasoning-only or tool-emitting summary is not a replacement
    // for history. Fail once without executing tools or resetting the chain.
    if (
      compacting &&
      (result.aborted ||
        result.patternMatch ||
    ...
    if (!compacting) {
      ...
      if (
        ...
        if (result.toolCalls.length > 0) {
          // Preserve the accepted tool call/result pair before switching to a
          // tool-free summary request. Never silently discard pending work.
