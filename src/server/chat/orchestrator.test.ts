...
describe('chat orchestrator', () => {
  ...
  describe('terminal cleanup of streaming messages', () => {
    ...
    it('stops consumption and records an error when a delta cannot be persisted', async () => {
      const eventStore = prepare()
      const append = eventStore.append.getMockImplementation()!
      eventStore.append.mockImplementation((sessionId, event) => {
        if (event.type === 'message.thinking') throw new Error('storage unavailable')
